...
def get_conf(conf):
    ...
    for section in config.sections():
        ...
        is_json_type = False
        if "Dtype" in config.options(section):
            val = config.get(section, "Dtype")
            if val == "json":
                is_json_type = True

        if is_json_type:
            for opt_name in config.options(section):
                # strip new lines
                json_str = ''.join(config.get(section, opt_name).split())
                val = json.loads(json_str)
                print("ok %s" % json.dumps(val, indent=2))
                section_values[opt_name] = val

        else:
            for opt_name in config.options(section):
                val = config.get(section, opt_name)
                if val == "":
                    val = None
                section_values[opt_name] = val
                pass
...
# data field validation toolkits
# @todo TODO
